...
// src/lib/env.ts
function getServerEnv(key, defaultValue = "") {
...
var FLUTTERWAVE_SECRET_KEY = getServerEnv("FLUTTERWAVE_SECRET_KEY");
var FLUTTERWAVE_CLIENT_ID = getServerEnv("FLUTTERWAVE_CLIENT_ID");
var FLUTTERWAVE_CLIENT_SECRET = getServerEnv("FLUTTERWAVE_CLIENT_SECRET");
var FLUTTERWAVE_ENVIRONMENT = getServerEnv("FLUTTERWAVE_ENVIRONMENT", "production");
var MARZPAY_SECRET_KEY = getServerEnv("MARZPAY_SECRET_KEY");
var MARZPAY_ENVIRONMENT = getServerEnv("MARZPAY_ENVIRONMENT", "production");
var RESEND_API_KEY = getServerEnv("RESEND_API_KEY");
var RESEND_FROM_EMAIL = getServerEnv("RESEND_FROM_EMAIL", "African Girl Rise <noreply@africangirlrise.org>");
var ADMIN_LOGIN_EMAIL = getServerEnv("ADMIN_LOGIN_EMAIL", "africangirlriseltd@gmail.com");
var ADMIN_LOGIN_PASSWORD = getServerEnv("ADMIN_LOGIN_PASSWORD");
var ADMIN_AUTH_SECRET = getServerEnv("ADMIN_AUTH_SECRET");
var SUPABASE_URL = getServerEnv("SUPABASE_URL");
var SUPABASE_ANON_KEY = getServerEnv("SUPABASE_ANON_KEY") || getServerEnv("SUPABASE_PUBLISHABLE_KEY");
var SUPABASE_SERVICE_ROLE_KEY = getServerEnv("SUPABASE_SERVICE_ROLE_KEY");
var isConfigured = {
...
function formatTextHeader(message) {
  ...
  rows = rows.flatMap((row2) => {
    let sepLen = maxKeyLength - row2.key.length;
    let prefix = `${row2.key}: ${" ".repeat(sepLen)}`;
    let emptyPrefix = `${" ".repeat(row2.key.length + 1)} ${" ".repeat(sepLen)}`;
    let foldedLines = foldLines(row2.val, 80, true).split(/\r?\n/).map((line) => line.trim());
...
// src/lib/resend.ts
var isResendConfigured = isConfigured.resend;
var resend = isResendConfigured ? new Resend(RESEND_API_KEY) : null;
var SENDER_EMAIL = RESEND_FROM_EMAIL;
...
var supabaseKey = process.env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_PUBLISHABLE_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
var sharedClientOptions = {
  auth: {
...
var isSupabaseAdminConfigured = Boolean(supabaseUrl && process.env.SUPABASE_SERVICE_ROLE_KEY);
...
function getIPFromHeader(forwardedFor) {
...
// api/contact.ts
var INJECTION_PATTERN = /(<script|javascript:|on\w+\s*=|\bDROP\b|\bUNION\b|\bSELECT\b|\bINSERT\b|\bDELETE\b|\bUPDATE\b|\bEXEC\b)/i;
...
var escapeHtml2 = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
var FORM_LABELS = { general: "\u{1F4AC} General Inquiry", mentor: "\u{1F393} Mentor Application", sponsor: "\u{1F496} Sponsorship Interest", donate: "\u{1F30D} Donation / Partnership" };
function safe(val) {
  return escapeHtml2(String(val || "").trim() || "\u2014");
}
function row(label, val) {
  const v = String(val || "").trim();
  if (!v) return "";
  return `<p><strong>${label}:</strong> ${escapeHtml2(v)}</p>`;
}
function sanitize(val, max = 255) {
  return String(val || "").trim().slice(0, max);
}
function buildAdminHtml(type, b) {
...
async function handler(req, res) {
  ...
  try {
    ...
    const rl = rateLimit(ip, 3, 60 * 60 * 1e3);
    if (rl.isLimited) {
    ...
    const message = sanitize(body.message, 1e4);
    ...
    const safeName = escapeHtml2(name || "Friend");
